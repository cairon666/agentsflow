...
func TestBuildPlanClassifiesCreateSkipConflict(t *testing.T) {
	...
	plan := BuildPlan("test", "project", map[string][]byte{
		createPath:   []byte("new"),
		skipPath:     []byte("same"),
		conflictPath: []byte("generated"),
	})
...
func TestBuildPlanWithManagedPathsAllowsJSONUpdateWithoutMarker(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "settings.json")
	if err := os.WriteFile(path, []byte(`{"model":"old"}`), 0o644); err != nil {
		t.Fatal(err)
	}
	plan := BuildPlanWithManagedPaths("claude", "project", map[string][]byte{
		path: []byte(`{"model":"new"}`),
